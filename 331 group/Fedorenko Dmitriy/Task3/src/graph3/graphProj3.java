...
public class graphProj3 extends Application {
	...
	public Graph pathToGraph(List<Vertex> path){
		Set<Edge> edges = new HashSet<Edge>();
		Set<Vertex> vertexes = new HashSet<Vertex>(); 
		for (int i = 0; i < path.size() - 1; i++) {
			vertexes.add(path.get(i));
	...
	public void start(Stage myStage) {
		...
		Button nnaBtn = new Button("NNA");
		Button dmstaBtn = new Button("DMSTA");
		...
		nnaBtn.setDisable(true);
		dmstaBtn.setDisable(true);
		...
		hospitals.add(new Vertex(10, 43.013159, 44.662113));
			...
			public void handle (final ActionEvent ae) {
				if (!latField.getText().isEmpty() && !lonField.getText().isEmpty()) {
					...
					if (!lat.isNaN() && !lon.isNaN() && (lat >= graph.minLat) 
						...
						nnaBtn.setDisable(false);
						dmstaBtn.setDisable(false);
		...
		//event handler for nnaBtn
		nnaBtn.setOnAction(new EventHandler<ActionEvent>() {
			public void handle (final ActionEvent ae) {
				Long time1 = System.nanoTime();
				Graph.PathAndObjects pao = graph.NearestNeighbourAlg(startVertex, hospitals);
				Long time2 = System.nanoTime();
				try {
					pathToGraph(pao.path).outputAdjacencyList("result/NNAPath.csv");
					graph.outputSVG("result/NNAPath.svg", pao.path, startCoord.x, startCoord.y, pao.objects);
					//graph.outputSVG("result/NNAPath.svg", pao.path, startCoord.x, startCoord.y, graph.DoubleMinSpanTreeAlg(startVertex, hospitals).objects, graph.minimumSpanningTree(objects));
				} catch (IOException e) {
					e.printStackTrace();
				}
				statusLabel.setText((time2 - time1)*1e-9 + " is elapsed");
		...
		//event handler for dmstaBtn
		dmstaBtn.setOnAction(new EventHandler<ActionEvent>() {
			public void handle (final ActionEvent ae) {
				Long time1 = System.nanoTime();
				Graph.PathAndObjects pao = graph.DoubleMinSpanTreeAlg(startVertex, hospitals);
				Long time2 = System.nanoTime();
				try {
					pathToGraph(pao.path).outputAdjacencyList("result/DMSTAPath.csv");
					graph.outputSVG("result/DMSTAPath.svg", pao.path, startCoord.x, startCoord.y, pao.objects);
					//graph.outputSVG("result/DMSTAPath.svg", pao.path, startCoord.x, startCoord.y, graph.DoubleMinSpanTreeAlg(startVertex, hospitals).objects, graph.minimumSpanningTree(objects));
				} catch (IOException e) {
					e.printStackTrace();
				}
				statusLabel.setText((time2 - time1)*1e-9 + " is elapsed");
		...
		nnaBtn.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
		dmstaBtn.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
        ...
        GridPane.setConstraints(tspLabel, 0, 8);
        GridPane.setConstraints(nnaBtn, 1, 8);
        GridPane.setConstraints(dmstaBtn, 2, 8);
        GridPane.setConstraints(latMinMaxLabel, 0, 9, 4, 1);
        GridPane.setConstraints(lonMinMaxLabel, 0, 10, 4, 1);
        GridPane.setConstraints(statusLabel, 0, 11, 4, 1);
        ...
        inputGridPane.getChildren().addAll(openMapLabel, openMapBtn, parseMapBtn, exportLabel, 
        		...
        		lonField, setStartBtn, dijkstraBtn, levitBtn, astarBtn, rb1, rb2, rb3, tspLabel, nnaBtn, dmstaBtn, latMinMaxLabel, 
		...
		Scene myScene = new Scene(rootGroup, 350, 330);
